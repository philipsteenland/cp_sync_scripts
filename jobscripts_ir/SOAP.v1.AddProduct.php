...
$Mag->Mag_Timestamp("C:\\xampp\\htdocs\\shop\\jobscripts_ir\\SOAP.v1.AddProduct.txt");
...
$rs = $Mag_Mssql->Mssql->Execute("SELECT TOP 100 items.itemcode AS ItemCode,
...
if($rs && $rs->_numOfRows > 0){			
	while (!$rs->EOF){		 
		...
		if(!$is_product){	
			...
			$newProductData['set'] = 4;
			
			
			if($rs->fields['status'] == 1){	
				$proxy->call($sessionId, 'product.create', array($producttype, 4, $rs->fields['ItemCode'], $newProductData));
			}
