...
$path = 'C:\\xampp\\htdocs\\shop\\jobscripts_ir\\';	
...
class MyDB extends SQLite3
{
    ...
    {
        $this->open('C:\\xampp\\htdocs\\shop\\jobscripts_ir\\data.db');
    }
}
...
//$results = $db->query('SELECT * from [order] WHERE creation_date = \'2012-01-09\'');
if($results){
while ($row = $results->fetchArray()) {
		...
		$i=1;
		$items = $db->query("SELECT p.sku as sku,quantity FROM [order] o LEFT JOIN order_line l ON o.order_id = l.order_id LEFT JOIN product p ON p.product_id = l.product_id and p.store_id = 29 WHERE o.order_id = ".$row['order_id']);
		if($items){
