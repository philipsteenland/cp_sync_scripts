...
$rs = $DBTransip->Execute("SELECT `table_id`,`field_type`,`sku`,`attribute_id`,`value`,`field_value`, `simples_skus` FROM `_viewItemsExact13`");	
...
if($rs && $rs->_numOfRows > 0){			
	while (!$rs->EOF){
		...
		if(isset($check[$rs->fields['field_type']])){
			...
			$row = $check[$rs->fields['field_type']]->fetch();
			
			//create value
			if(!$row && $rs->fields['field_value'] == null && isset($check[$rs->fields['field_type']])){
				
			}
			
			if($row or $rs->fields['field_value']==null) {
